...
/* Keep off the main thread stack (inference + USB already use a lot). */
				  ...
				  size_t box_count, const uint8_t *thumb, size_t thumb_len)
{
	...
	if (last_send_ms != 0 && (now - last_send_ms) < min_interval_ms) {
		LOG_DBG("Companion debounced (%ds min interval)",
			CONFIG_COMPANION_MIN_INTERVAL_SEC);
		return 1;
	}
	...
	if (err) {
		LOG_WRN("Companion frame encode failed: %d", err);
		return err;
	}
